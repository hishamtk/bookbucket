				...
				<div id="productBox">
					<?php
						if (isset($_GET['proId'])) {
							...
							$proId=$_GET['proId'];
							$getProduct="SELECT * FROM products WHERE productId=$proId";
							...
							while($rowProduct=mysqli_fetch_array($runProduct)){
								...
								$productDesc=$rowProduct['productDesc'];

								echo "

									<div id='singleProductDetails'>
										<img src='adminArea/productImages/$productImage' style='width:180px;height:220px;'/>
										...
										<h6>&#8377;$productPrice</h6>
										<p>$productDesc</p>
										<a href='index.php?proId=$productId' style='float:right;'><button>Add to Cart</button></a>
										<a href='index.php'><button>Go back</button>
									</div>
								";
							}
						}
